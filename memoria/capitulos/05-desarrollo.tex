...
El cuadro~\ref{tab:config-ab} recoge la configuración nominal que comparten los dos runs, idéntica salvo en la lógica de variación.
...
Antes de comparar el fitness conviene examinar la forma de los mazos que produce cada variante. El cuadro~\ref{tab:estructura-ab} reúne las métricas estructurales de la población final en ambos runs. Los dos cumplen las restricciones formales del formato: los 40 mazos de la población final tienen exactamente 60 cartas y ninguno viola la norma de un máximo de cuatro copias por carta. Hasta ahí la equivalencia. A partir de ahí, la diferencia es pronunciada.
...
El baseline respeta la norma 4-of en sentido formal: ningún mazo tiene más de cuatro copias de una carta. Pero el espíritu de la norma se viola. Con una media de 3,55 \emph{singletons} no-tierra por mazo, muchos individuos acumulan cinco u ocho cartas en copia única que funcionan como piezas \emph{tech} sueltas. Los operadores originales, al modificar de copia en copia, generaban y regeneraban esos residuos más rápido de lo que la función de consolidación de \emph{singletons} podía eliminarlos.
...
El cuadro~\ref{tab:fitness-ab} compara los estadísticos de fitness de ambos runs en la ventana de generaciones disponible para los dos.
...
Cuando el experimento anterior confirmó que los operadores pack-aware funcionaban, apareció un efecto colateral que no era obvio de prever: la mejora estructural de los mazos hizo que una parte de la función de fitness dejara de discriminar. El problema es de naturaleza estadística. Se detecta auditando el coeficiente de variación (CoV) de cada componente de la métrica de calidad sobre la población final del run pack-aware.

\subsection{Diagnóstico: saturación de componentes}

La métrica de calidad combinaba seis componentes con pesos distintos. Antes del pack-aware, esas componentes medían propiedades que podían variar ampliamente de un mazo a otro. Después del pack-aware, tres de ellas convergieron a un valor constante para prácticamente toda la población, como evidencia el cuadro~\ref{tab:cov-quality}.

\begin{table}[H]
\centering
\caption{Análisis de varianza de las seis componentes de la métrica de calidad sobre los 40 mazos finales del run pack-aware (gen 30). CoV $<$ 10\,\% indica saturación.}
...
La solución es directa. Se eliminan las tres componentes saturadas del cálculo de la calidad y se reponderan las tres que sí discriminan, según muestra el cuadro~\ref{tab:quality-pesos}.
...
El cuadro~\ref{tab:config-final} resume los parámetros con los que se ejecutó el run final.

\begin{table}[H]
\centering
\caption{Parámetros del run final.}
...
El cuadro~\ref{tab:fitness-final} recoge los estadísticos de fitness en generaciones representativas del run final.
...
El gauntlet consta de cinco mazos tier-1 del Standard de la era Final Fantasy (junio-julio 2025), elegidos de fuentes públicas (Pro Tour Final Fantasy y MTGGoldfish) para cubrir los grandes arquetipos del meta, según recoge el cuadro~\ref{tab:gauntlet-anchors}.
